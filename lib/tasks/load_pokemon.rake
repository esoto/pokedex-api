...
namespace :pokedex do
  ...
  task :load_pokemon => [:environment] do
    ...
    pokemon_json.each do |pokemon|
      ...
      Pokemon.where(number: pokemon[0], name: pokemon_data['name'], attack: pokemon_data['attack'], defense: pokemon_data['defense'], pokemon_type:pokemon_data['type']).first_or_create
    end
  end
end
